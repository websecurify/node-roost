var path = require('path');
var logsmith = require('logsmith');

// ---

var sync = require(path.join(__dirname, 'sync.js'));
var packages = require(path.join(__dirname, 'packages.js'));
var services = require(path.join(__dirname, 'services.js'));
var commands = require(path.join(__dirname, 'commands.js'));
var bootstrap = require(path.join(__dirname, 'bootstrap.js'));

// ---

function launch(opt, manifest, plugins, target, callback) {
	logsmith.silly('executing roost plugins');
	
	(plugins || []).forEach(function (plugin) {
		plugin.roost(opt, manifest, target);
	});
	
	logsmith.silly('executing roost workflow');
	
	sync.roost(opt, manifest, target);
	bootstrap.roost(opt, manifest, target);
	packages.roost(opt, manifest, target);
	services.roost(opt, manifest, target);
	commands.roost(opt, manifest, target);
	
	target.on('error', function (error) {
		callback(error);
	});
	
	target.on('complete', function () {
		callback();
	});
	
	logsmith.silly('igniting target');
	
	target.ignite(opt.options.dry);
}

// ---

exports.launch = launch;
